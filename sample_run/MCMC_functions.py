...
class MODEL:
    ...
    def create_swm_file(self, nlayers, create_tvel_file=False):
        ...
        nmsublayers = np.zeros(self.nmantle+1)
...
def maxtwo (number1, number2):
	...
	return comparmax
# ----------------------------------------------------------------------------
def startmodel (hmin,nintf,totch,nl,maxz,vmin,vmax,cvmin,cvmax,cavgV,chmin,
                chmax,epimin,epimax,otmin,otmax,hypmin,hypmax,nevts):
...
def runmodel_bw (x,phases):
    ...
    return (dpre_bw)
# ----------------------------------------------------------------------------
def startchain (x,errorflag1,stz,stvel,stepi,stotime,sthyp,vmin,vmax,cvmin,
                cvmax,cavgV,chmin,chmax,epimin,epimax,otmin,otmax,hypmin,
                hypmax,hmin,maxz,chain):
	nmantle = copy.deepcopy(x.nmantle)
	if errorflag1 == 'on':
		INTF = np.zeros((nmantle+1))
	...
	return (x,errorflag1)
# ----------------------------------------------------------------------------
def randINTF (vmin,vmax,chmin,hmin,maxz,F,vsIN,thetaV2):
	nintf = len(F)
	...
	if matching == 0: #crustal case perturb bottom half instead
            ...
            delv2 = vsIN[1]-maxtwo(vmin,vsIN[0])
            vsOUT[2:] = vsIN[1:]
        else:
            ...
            #                                  vsIN[matching])
            delv2 = vsIN[matching]-maxtwo(vmin,vsIN[matching-1])
	...
	# If the velocity of the newly added layer is outside bounds or 
        ...
        if (matching == 0):
            if ((vsOUT[1] > vmax) or (vsOUT[1] < vmin) or (vsOUT[1] < vsOUT[0])
                or (vsOUT[1] > vsOUT[2])):
	...
	return (r, newF, vsOUT, WARN_BOUNDS, matching, delv2)
...
# ----------------------------------------------------------------------------
def vdispfig(rep_cnt,repeat,nummods,revPHIind,keptPHI,CHMODS,scalarMap,dobs,instpd,Ult_ind,weight_opt,wsig,pmin,pmax,vmin,vmax,CS3,maxz_m,abc,run,SAVEF):
	plt.close('all')
	# loop through all kept models plotting one at a time
	jj=0
	while (jj < nummods):
		...
		linebest = plt.plot(pltdpre, instpd, c=colorVal, linewidth=2.0, zorder=3)
		jj = jj + 1
	
	# Plot ultimate best model of all chains
	curmod = copy.deepcopy(CHMODS[Ult_ind])
	pltdpre = curmod.dpre
	ULTl, = plt.plot(pltdpre, instpd, 'k--', linewidth=4.0, 
					zorder=3, label='model with lowest \n prediction error')
	cb=plt.colorbar(CS3)
	cb.set_label(r'$\phi$(m)', labelpad=-65)
	plt.xlabel('VS (km/s)')
	plt.ylabel('Period (s)')
	plt.axis([vmin, vmax, pmin, pmax])
	ax = plt.gca()
	ax.set_ylim(ax.get_ylim()[::-1])
	plt.grid(True,linestyle='-',color='0.75',zorder=0)
	obs, = plt.plot(dobs, instpd, 'ko', zorder=10, markersize=7, 
					label='measured group velocities')
	if weight_opt == 'ON':
		obserr = plt.errorbar(dobs, instpd, xerr=wsig, zorder=20, linestyle="None", linewidth=2.0, ecolor='k')
	plt.legend(handles=[ULTl, obs], fontsize=10, loc='lower left')
	if rep_cnt == (repeat-1):	
