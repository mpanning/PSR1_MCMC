...
MAIN = '/Users/panning/work_local/InSightWorkshop/sample_run'
os.chdir(MAIN)
# MAIN = os.getcwd()
...
# Read in arrival time data into a list of arrays
bwtimes = []
phases = []
dobs_bw = []
...
#                                            'formats': ('|S10', 'f')}))
        bwtimes.append(np.genfromtxt(bwfiles[i],dtype=None,encoding='ascii',
                                     names=['phase', 'time']))
...
while (run < numrun):
        ...
        # interfaces + 2 (the crust-mantle interface and CMB). 
        ...
        # Bounds on mantle velocities, can be depth dependent
        ...
        # Set up totch chains:------------------------        
        # Currently inconsistent with depth dependent ranges in MCMC_functions
        # FIX THIS
        stz,stvel,stepi,stotime,sthyp = startmodel(hmin,nintf_init,totch,
                                                   nv_init,maxz,vrad,vmin,vmax,
        ...
        while (chain < totch):
                ...
                x,errorflag1=startchain(x,errorflag1,stz,stvel,stepi,stotime,
                                        sthyp,vrad,vmin,vmax,cvmin,cvmax,cavgV,
                ...
                for i in range(0,nevts):
                        ...
                        dpre_sw.append(np.zeros(int(tnum[i])))
                ...
                if errorflag1 == 'on':
                        ...
                        print("no disp.out file found, ")
                        print("try to re-do start of chain ")
                        print(" ")
                        errorflag1 = 'off'
                else:
                        print(PHI)
