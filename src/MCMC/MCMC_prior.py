...
totch = 5  # Total number of chains
numm = 20000  # Number of iterations per chain
...
while (run < numrun):
	...
	while (chain < totch):
		...
		# For prior calculation, this isn't needed
		# Run sw model
		...
		#				       diagCE,weight_opt)
		# Set PHI to dummy value for prior distribution
		PHI[0] = 1.0
		x.PHI = 1.0
		...
		while (k < (numm-1)):
			...
			# Change an interface depth	
				...
				if not (pI == 0): #check layer above
					depth = intfOUT[pI-1]
					vmin1 = (vmin[0] + (vmin[1] - vmin[0]) 
						 * depth/maxz) 
					vmax1 = (vmax[0] + (vmax[1] - vmax[0]) 
						 * depth/maxz) 
					if ((vsOUT[pI] < vmin1) or 
					    (vsOUT[pI] > vmax1)):
						...
						print ('Automatically REJECT' +
						       ' model')
						WARN_BOUNDS = 'ON'
				if not (pI == nintf - 1): #check layer below
					depth = intfOUT[pI + 1]
					vmin1 = (vmin[0] + (vmin[1] - vmin[0]) 
						 * depth/maxz) 
					vmax1 = (vmax[0] + (vmax[1] - vmax[0]) 
						 * depth/maxz) 
					if ((vsOUT[pI + 2] < vmin1) or 
					    (vsOUT[pI + 2] > vmax1)):
						print ('!!! Velocity outside' +
						       ' bounds!!!')
						print ('Automatically REJECT' +
			...
			# Accept all models in bounds for prior distribution
			PHI[k+1] = 1.0
			pac = 1.0
			q = 0.0
			if pac < q:
