...
# Serious documentation is necessary when this is ready to go
# -----------------------------------------------------------------------------
...
MAIN = os.getcwd()
# MAIN = '/home/mpanning/MCMC/test_run4'
# os.chdir(MAIN)
run_name = os.path.basename(MAIN) 

# Tuneable parameters moved to the beginning for more reliable editing
# --- Markov Chain details
totch = 1  # Total number of chains
numm = 10000  # Number of iterations per chain
BURN =  1999 # Number of models designated BURN-IN, gets discarded
M = 10  # Interval to keep models (e.g. keep every 100th model, M=100)
repeat = 1  # Number of times to run the chains
...
# --- Bounds on mantle velocities: array lists vmin and vmax at top and 
# bottom of model.  Best to keep constant delv, though.
# REDO THIS WITH MULTIPLE LAYERS AND VARIABLE DELV

# Flexible parameterization of velocity priors
prior_vrad = np.array([6371.1, 3479.9])
prior_vmin = np.array([3.0, 6.0]) 
prior_vmax = np.array([6.0, 9.0])
prior_delv = prior_vmax - prior_vmin #This now is variable by depth
max_neg_grad = -0.0005 # units of km/s per km
#if not ((vmax[1] - vmin[1]) == delv):
#	raise ValueError('delv not constant')
...
fnum = np.zeros(nevts,dtype=np.int_)
...
layopt = ([3])  # Shouldn't this be random?
...
# The following are more tuneable options less likely to be changed than thos
# moved to the beginning of the code
...
while (run < numrun):
	...
	# interfaces + 2 (the crust-mantle and mantle-core interfaces). 
	# nintf_init= nmantle_init + 1
	nintf_init= nmantle_init + 2
	...
	while (chain < totch):
		...
		x,errorflag1=startchain(x,errorflag1,stz,stvel,stepi,stotime,
					sthyp,prior_vrad,prior_vmin,prior_vmax,
					cvmin,cvmax,cavgV,chmin,chmax,epimin,
					epimax,otmin,otmax,hypmin,hypmax,hmin,
					maxz,chain,max_neg_grad,planet_radius)		
		
		
		# Create swm and nd input files
		try:
			x.create_swm_file(swm_nlay, create_tvel_file=True)
		except ValueError:
		...
		gv_pre = []
		tnum = np.zeros(nevts, dtype=np.int_)
		dpre_sw = []
		...
		try:
			...
			print 'Taup unable to calculate arrivals for model'
			print 'Restart chain'
			errorflag1 = 'on'
			continue
		...
		while (k < (numm-1)):
			...
			# save original velocities and interfaces
			...
			if pDRAW == 2:
				...
				if pV == 0:
					...
					# mantle must be faster than crust
					...
					vmin = (vmin1 + (vmin2 - vmin1)
						*(radius - rad1)/(rad2 - rad1))
					vmax = (vmax1 + (vmax2 - vmax1)
						*(radius - rad1)/(rad2 - rad1))

					#vmin1 = (vmin[0] + (vmin[1] - vmin[0]) 
					#	 * depth/maxz) 
					#vmax1 = (vmax[0] + (vmax[1] - vmax[0]) 
					#	 * depth/maxz) 
					...
					# velocity range or creates a negative
					# velocity gradient - REJECT
					# Maintain limit on negative gradients
					# for base layer (need rays to turn up)
					...
					#vmax1 = (vmax[0] + (vmax[1] - vmax[0]) 
					#	 * depth/maxz) 
					# Calculate gradients to points above
					# and below
					grad1 = ((vsOUT[pV] - vsOUT[pV-1])/
						 (intfIN[pV-1] - intfIN[pV-2]))
					grad2 = ((vsOUT[pV+1] - vsOUT[pV])/
						 (intfIN[pV] - intfIN[pV-1]))
					...
					# velocity range or creates a negative
					# velocity gradient - REJECT
					# Use this for no neg grads
					# if ((vsOUT[pV] < vsOUT[pV-1]) or 
					#    (vsOUT[pV] > vsOUT[pV+1]) or
					#    (vsOUT[pV] < vmin) or 
					#    (vsOUT[pV] > vmax)):
					# Use this to allow any grad
					# if ((vsOUT[pV] < vmin) or 
					#    (vsOUT[pV] > vmax)):
					if ((vsOUT[pV] < vmin) or 
					    (vsOUT[pV] > vmax) or 
					    (grad1 < max_neg_grad) or
					    (grad2 < max_neg_grad)):
						print ('!!! Outside velocity ' +
						       'range allowed!!')
						print ('Automatically REJECT ' +
			...
			# Change an interface depth	
				...
				# Check that perturbed layer and layers above 
				# and below still have velocities in bounds
				if not (pI == 0 or WARN_BOUNDS == 'ON'): 
                                        #check layer above
					depth = intfOUT[pI-1]
					radius = planet_radius - depth
					rad1 = prior_vrad[prior_vrad >= radius][-1]
					...
					vmax = (vmax1 + (vmax2 - vmax1)
						*(radius - rad1)/(rad2 - rad1))
					grad = ((vsOUT[pI+1]-vsOUT[pI])/
						(intfOUT[pI]-intfOUT[pI-1]))
					if ((vsOUT[pI] < vmin) or 
					    (vsOUT[pI] > vmax) or
					    (grad < max_neg_grad)):
						print ('!!! Velocity outside' +
						       ' bounds!!!')
						print ('Automatically REJECT' +
						       ' model')
						WARN_BOUNDS = 'ON'

				if not (pI == nintf - 1 or WARN_BOUNDS == 'ON'):
                                        #check layer below
					depth = intfOUT[pI + 1]
					radius = planet_radius - depth
					rad1 = prior_vrad[prior_vrad >= radius][-1]
					...
					vmax = (vmax1 + (vmax2 - vmax1)
						*(radius - rad1)/(rad2 - rad1))
					grad = ((vsOUT[pI+2]-vsOUT[pI+1])/
						(intfOUT[pI+1]-intfOUT[pI]))
					if ((vsOUT[pI + 2] < vmin) or 
					    (vsOUT[pI + 2] > vmax) or
					    (grad < max_neg_grad)):
						print ('!!! Velocity outside' +
						       ' bounds!!!')
				...
				depth = intfOUT[pI]
				radius = planet_radius - depth
				if not (WARN_BOUNDS == 'ON'):
					rad1 = prior_vrad[prior_vrad >= radius][-1]
					print "Error: depth radius prior_vrad", depth, radius, prior_vrad
					rad2 = prior_vrad[prior_vrad <= radius][0]
					vmin1 = prior_vmin[prior_vrad >= radius][-1]
					vmin2 = prior_vmin[prior_vrad <= radius][0]
					vmax1 = prior_vmax[prior_vrad >= radius][-1]
			...
			# Create a new layer
				...
				if newNM > nmax:
					...
					# Get the current model's interface
					# depths
					# intfIN = np.zeros(curNM+2)
					# intfIN = oldx.radius - oldx.mantleR
					# F = np.zeros(curNL-1)
					...
					# Generate an interface at a random
					# depth, but have check measures in 
					# place so that the interface cannot 
					# be within hmin of any of the 
					# existing interfaces
					(addI, intfOUT, vsOUT, WARN_BOUNDS,
					 BDi, delv,delv2) = randINTF(prior_vrad,
								     prior_vmin,
								     prior_vmax,
								     prior_delv,
								     chmin,hmin,
			...
			try:
				...
				print ('Taup unable to calculate arrivals ' +
				       'for model')
				print 'Redo iteration'
				del x
				continue
