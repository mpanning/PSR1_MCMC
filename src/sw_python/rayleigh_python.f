c      program rayleigh
      subroutine rayleigh(epsarg,npow,dt,fnyquist,nbran,cmin,cmax,
     * maxlyr,modelfile,modearray,nmodes)
...
c compile:  f77 -static rayleigh.f ltable.r
c or g77 -fno-automatic rayleigh.f ltable.f
c source text of several subroutines is ratfor, in ltable.r

c note on normalization:
c love and rayleigh produce eigenvectors normalized such that
c ekin = \omega^2 \int [U^2 + l(l+1)V^2] r^2 dr = 1  (Rayleigh)
c ekin = \omega^2 \int l(l+1) W^2 r^2 dr = 1 (Love)

c this differs by omega^2 from Dahlen and Tromp, or Breviary Chap 9.

c Make sure MAXL (the maxmimum number of radial points) is consistent through
c all subroutines (including ltable)!
      parameter (MAXL=1500)
      parameter (MAXMODES=1000)
      character*256 filnam
      character*256 modelfile
...
c old version with stdin input
c      print *,'enter input model file: '
c      read(*,'(a256)') filnam
c      open(7,file=filnam,status='old',form='formatted')
c      print *,'enter output file: '
c      read(*,'(a256)') filnam
c      open(8,file=filnam,form='formatted')
c      print *,'enter eigfn output file: '
c      read(*,'(a256)') filnam
c      open(9,file='dummyeig',form='unformatted')
c      call modeln(7,8,9)
c      close(7)
c      close(9)
c      open(9,file=filnam,form='unformatted')
c      call ltable(8,9,1)
c      close(8)
c      close(9)
c     New version with input from rayleigh.dat
c F2PY should make info from rayleigh.dat passed to subroutine
c F2PY this includes model file, and output files in this code block
c F2PY maybe skip output files in first version?
c      open(unit=ifin,file='rayleigh.dat',status='old',form='formatted')
c     comment line and input model file
c      read(ifin,*)
c      read(ifin,fmt='(a256)') filnam
      open(7,file=modelfile,status='old',form='formatted')
c     comment line and ASCII output file
c      read(ifin,*)
c      read(ifin,fmt='(a256)') filnam
c      open(8,file=filnam,form='formatted')
c     comment line and eigfn binary output file
c      read(ifin,*)
c      read(ifin,fmt='(a256)') filnam
c      open(9,file='dummyeig',form='unformatted')
...
c**** of the secular determinant as det and the count of zero crossings.
      ...
      common r(MAXL),fmu(MAXL),flam(MAXL),qshear(MAXL),qkappa(MAXL),
     + xa2(MAXL),xlam(MAXL),rho(MAXL),qro(3,MAXL),g(MAXL),qg(3,MAXL),
     + fcon(MAXL),fspl(3,MAXL),lcon(MAXL),lspl(3,MAXL),ncon(MAXL),
     + nspl(3,MAXL),ccon(MAXL),cspl(3,MAXL),acon(MAXL),aspl(3,MAXL)
      common/bits/pi,rn,vn,wn,w,wsq,wray,qinv,cg,tref,fct,eps,fl,
     +  fl1,fl2,fl3,sfl3,nord,l,kount,knsw,ifanis,iback
      common/eifx/a(5,MAXL),dcda(MAXL),dcdb(MAXL),dcdrh(MAXL),
     +     dcdxi(MAXL),
...
c*** massages spheroidal mode eigenfunctions before output ***
     ...
     + nspl(3,MAXL),ccon(MAXL),cspl(3,MAXL),acon(MAXL),aspl(3,MAXL)
      common/bits/pi,rn,vn,wn,w,wsq,wray,qinv,cg,tref,fct,eps,fl,
     +  fl1,fl2,fl3,sfl3,nord,l,kount,knsw,ifanis,iback
      common/eifx/a(5,MAXL),inorm(MAXL),kdum(22*MAXL)
      common/rindx/nic,noc,nsl,nicp1,nocp1,nslp1,n
      dimension zi(4)
...
c     (in funny units) and scale V, dV/dr by sfl3.
   ...
   95 a(2,1)=1.5d0*a(1,2)/r(2)-.5d0*a(2,2)
     ...
     + nspl(3,MAXL),ccon(MAXL),cspl(3,MAXL),acon(MAXL),aspl(3,MAXL)
      common/bits/pi,rn,vn,wn,w,wsq,wray,qinv,cg,tref,fct,eps,fl,
     +  fl1,fl2,fl3,sfl3,nord,l,kount,knsw,ifanis,iback
      common/eifx/a(5,MAXL),kdum(23*MAXL)
      common/rindx/nic,noc,nsl,nicp1,nocp1,nslp1,n
      dimension f(8),zi(4)
      do 5 i=1,8
    5   f(i)=0.
...
c*** given rho and spline coeffs,computes gravity ***
      implicit real*8(a-h,o-z)
      parameter (MAXL=1500)
      dimension g(MAXL),rho(MAXL),qro(3,MAXL),r(MAXL)
      g(1)=0.d0
      do 10 i=2,n
        im1=i-1
        del=r(i)-r(im1)
...
c*** value of the exponent at the starting level ls.
     ...
     +  fl1,fl2,fl3,sfl3,nord,l,kount,knsw,ifanis,iback
      common/rindx/nic,noc,nsl,nicp1,nocp1,nslp1,n
      dimension rrlog(MAXL),p(MAXL),v(MAXL)
      save rrlog,p,ifirst
      data ifirst/1/
...
c      subroutine modeln(iin,iout,ioeig)
      subroutine modeln(iin)
      implicit real*8(a-h,o-z)
      parameter (MAXL=1500)
      integer*4 ititle(20)
     ...
     + nspl(3,MAXL),ccon(MAXL),cspl(3,MAXL),acon(MAXL),aspl(3,MAXL)
      common/bits/pi,rn,vn,wn,w,wsq,wray,qinv,cg,tref,fct,eps,fl,
     +  fl1,fl2,fl3,sfl3,nord,l,kount,knsw,ifanis,iback
      common/eifx/vpv(MAXL),vph(MAXL),vsv(MAXL),vsh(MAXL),eta(MAXL),
     + kdum(23*MAXL)
      common/rindx/nic,noc,nsl,nicp1,nocp1,nslp1,n
      common/arem/wrk(3*MAXL)
...
c*** polynomial model ***
 ...
 2000 if(ifanis.ne.0) go to 3000
   ...
   25 eta(i)=1.d0
 3000 continue
c 3000 if(iout.lt.0) goto 30
*** write out model ***
c F2PY no need to write, set iout = -1
c      write(iout,900) (ititle(k),k=1,20),tref
c  900 format(1x,20a4,' ref per =',f6.1,' secs',///,2x,'level',
c     1 4x,'radius(m)',5x,'rho(kg/m3)',2x,'vpv(m/s)',4x,'vph(m/s)',4x,
c     2 'vsv(m/s)',4x,'vsh(m/s)',4x,'eta',9x,'qmu ',8x,'qkap',/)
...
c        write(iout,*) 'corrected for WARNING as follows:'
c	write(iout,*) 'r,rho,vpv,vsv,vph,vsh multiplied by 1000 to'
c	write(iout,*) 'convert to MKS units.'
      endif
      nsl=n
...
c      rh4=rho(i)
c      q4=qshear(i)
c75    write(ioeig) rn4,vp4,vs4,rh4,q4
...
c NOTE, actually normalized by r(n), so useful for other planet models
...
c                                                     -rpb
      parameter (MAXL=1500)
      dimension x(MAXL),y(MAXL),q(3,MAXL),f(3,MAXL),yy(3)
      equivalence (yy(1),y0)
      data yy/3*0.d0/
      j1=i1+1
      y0=0.d0
...
c      common/mtab/we(1200),de(1200),ke(1200),wtry(600),bm(600),um(600)
      common/mtab/we(9600),de(9600),ke(4800),wtry(4800),bm(4800) 
     +     ,um(4800)
